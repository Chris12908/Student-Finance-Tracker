...
class FinanceTracker {
    ...
    async init() {
        ...
        try {
            ...
            await this.loadData();
            // Load saved categories into state (if any)
            try {
                const savedCategories = this.storage.loadCategories();
                if (Array.isArray(savedCategories) && savedCategories.length > 0) {
                    this.state.setCategories(savedCategories);
                }
            } catch (err) {
                console.warn('Failed to load saved categories, using defaults', err);
            }
    ...
    setupSettingsEventListeners() {
        ...
        // Save settings buttons (support multiple locations)
        document.querySelectorAll('.save-settings-btn, #save-settings-btn').forEach(btn => {
            btn.addEventListener('click', () => this.saveAllSettings());
        });
    }

    /**
     * Save all settings (currency, exchange rates, theme, categories, budget)
     */
    saveAllSettings() {
        // Base currency
        ...
        // Theme
        const theme = document.documentElement.getAttribute('data-theme') || (this.storage.loadSettings().theme || 'light');
        ...
        if (!isNaN(budgetCap) && budgetCap >= 0) {
            this.state.setBudgetCap(budgetCap);
            this.storage.saveBudgetCap(budgetCap);
        }
    ...
    updateBudgetProgress(currentSpending) {
        ...
        if (currentSpending > budgetCap) {
            ...
            this.showMessage(`⚠️ You've exceeded your monthly budget by ${this.formatCurrency(Math.abs(remaining))}!`, 'alert');
        } else {
            ...
            if (percentage > 90) {
                this.showMessage(`⚠️ You're close to your budget limit. ${this.formatCurrency(remaining)} remaining.`, 'alert');
            }
        }
    }
    ...
    updateCategoryOptions() {
        ...
        if (!categorySelect) return; // safety: form may not be rendered yet
    ...
    updateCategoryList() {
        ...
        if (!categoryList) return; // safety: settings UI may not be rendered

        categoryList.innerHTML = '';
        categories.forEach(category => {
            const tag = document.createElement('div');
            tag.className = 'category-tag';
            tag.innerHTML = `${category} <button class="category-remove" aria-label="Remove ${category} category" onclick="app.removeCategory('${category}')">×</button>`;
    ...
    setTheme(theme) {
        ...
        if (themeIcon && themeText) {
            // Use text-only labels for accessibility and remove emoji characters.
            // Icon styling is handled in CSS (no emoji glyphs are inserted programmatically).
